...
/**
 * Get x402 paywall configuration for session-based payments
 */
...
/**
 * Build an unsigned coin transfer transaction for the client to sign.
 * This enables server-side transaction building to avoid CORS issues with Movement RPC.
 * 
 * @param senderAddress - The wallet address of the sender (viewer)
 * @param amount - The amount in octas (smallest unit)
 * @param recipientAddress - The wallet address of the recipient (creator or platform)
 * @returns Promise with unsigned transaction BCS bytes as base64
 */
export async function buildUnsignedTransaction(
  ...
  // Use recipient address if provided, otherwise fall back to platform address
  const payToAddress = recipientAddress || MOVEMENT_PAY_TO;
  const normalizedPayTo = normalizeAddress(payToAddress);
  
  console.log(`[x402] Building unsigned transaction:`);
  console.log(`  - Sender: ${normalizedSender}`);
  console.log(`  - Pay To: ${normalizedPayTo}`);
  console.log(`  - Amount: ${amount} octas`);
  
  // Build the transaction using server's Aptos client (no CORS issues)
  const transaction = await aptos.transaction.build.simple({
    sender: normalizedSender,
    data: {
      function: "0x1::aptos_account::transfer_coins",
      typeArguments: ["0x1::aptos_coin::AptosCoin"],
      functionArguments: [normalizedPayTo, amount],
    },
  });
  
  // Serialize the raw transaction to BCS bytes
  const rawTxBytes = transaction.rawTransaction.bcsToBytes();
  const base64 = Buffer.from(rawTxBytes).toString('base64');
  
  console.log(`[x402] Built unsigned transaction: ${rawTxBytes.length} bytes`);
...
/**
 * Submit a signed transaction to Movement testnet.
 * This combines the raw transaction with the account authenticator and submits it.
 */
async function submitTransactionToMovement(
  rawTxBytes: Uint8Array,
  signatureBytes: Uint8Array
): Promise<{ success: boolean; txHash: string }> {
  try {
    console.log('[x402] Deserializing transaction and signature...');
    console.log(`[x402] Raw tx bytes: ${rawTxBytes.length}B, Signature bytes: ${signatureBytes.length}B`);
    
    // Deserialize the raw transaction and authenticator
    const rawTxDeserializer = new Deserializer(rawTxBytes);
    const rawTransaction = RawTransaction.deserialize(rawTxDeserializer);
    
    // Deserialize the authenticator - wallet returns TransactionAuthenticator directly
    const sigDeserializer = new Deserializer(signatureBytes);
    const txAuthenticator = TransactionAuthenticator.deserialize(sigDeserializer);
    
    console.log('[x402] Building signed transaction...');
    
    // Create a SignedTransaction combining raw transaction and authenticator
    const signedTransaction = new SignedTransaction(rawTransaction, txAuthenticator);
    ...
    console.log(`[x402] Signed transaction bytes: ${signedTxBytes.length}B`);
    console.log('[x402] Submitting to Movement testnet via RPC...');
    
    // Submit the signed transaction using the raw submit endpoint
    const response = await fetch(`${MOVEMENT_RPC_URL}/transactions`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error(`[x402] RPC submission failed: ${response.status} - ${errorText}`);
      return { success: false, txHash: '' };
    }
    
    const result = await response.json() as { hash: string };
    console.log(`[x402] Transaction submitted: ${result.hash}`);
    ...
    console.log(`[x402] Transaction confirmed: ${confirmedTx.hash}`);
    ...
    console.error('[x402] Transaction submission failed:', error);
    return { success: false, txHash: '' };
  }
}

/**
 * Verify a transaction on the Movement blockchain.
...
async function verifyTransactionOnChain(
  ...
  try {
    console.log(`[x402] Verifying transaction on-chain: ${txHash}`);
    ...
    // For now, we trust that if the tx was submitted and succeeded, it's valid
    // Full verification would check the exact amount and recipient in the payload
...
export async function verifyPaymentHeader(
  ...
  try {
    ...
    // Submit directly to Movement testnet
    const result = await submitTransactionToMovement(transactionBytes, signatureBytes);
