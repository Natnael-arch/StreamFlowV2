...
export function useX402(): UseX402Return {
  ...
  const submitPaymentWithWallet = useCallback(async (
    ...
    try {
      ...
      console.log('[x402] Submitting coin transfer via wallet adapter:', {
        to: normalizedPayTo,
        amount: requirements.maxAmountRequired,
      });

      // Build the transfer transaction payload using adapter format
      const payload = {
        data: {
          function: "0x1::aptos_account::transfer_coins" as `${string}::${string}::${string}`,
          typeArguments: ["0x1::aptos_coin::AptosCoin"],
          functionArguments: [normalizedPayTo, requirements.maxAmountRequired],
        },
      };
